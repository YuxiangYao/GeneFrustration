...
class boolnetsys {
  ...
  int OEKD_GotoStable(IntegerVector gene,int n_control,std::set<int> &gene_code,int *EachID,
      IntegerVector markxx,int nx);
...
int boolnetsys::OEKD_GotoStable(IntegerVector gene,int n_control,std::set<int> &gene_code, int *EachID, IntegerVector markxx,int nx){
...
List Pressure2Potential(IntegerMatrix AdjMat,IntegerMatrix OriStabSta,List MarkerList,
  ...
  for(ii=0;ii<Ncell;ii++){
    ...
    NewTime[ii]=InducedCell.OEKD_GotoStable(ControlSet,n_control,OEKDers,
        TempID,mark_ps,Nps);
